...
class DIJET:
	def __init__(self, nreplica=1, lambdaIR=0.3, deta=0.05, gauss_param=0.0): 
		...
		self.dlogr = 0.01
		self.logr_values = np.arange(-13.8, round(np.log(1/lambdaIR), 2), self.dlogr)
	...
	def load_basis_dipoles(self):
		...
		unpolar_input_file = current_dir + '/dipoles/narr_ymin4.61_ymax9.81_AAMS09.dat'
	...
	def fourier_bessel(self, kvar, indices_array, amp, IR_reg = [None, 0]):
		...
		if amp == 'N':
			...
			measure = self.dlogr*(r**2) # dln(r) = 0.01 from evolution code
			...
			# for i, lnr in enumerate(self.logr_values[1490:]):
				# print(i+1490, np.exp(lnr), maxr_index)

			target_y_index = round((1/self.dlogr)*(np.log(1/x) - 4.61))
			print(maxr_index, target_y_index)
